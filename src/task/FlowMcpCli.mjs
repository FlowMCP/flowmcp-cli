...
class FlowMcpCli {
    ...
    // ---------------------------------------------------------------------
    // Memo 051 — sqlite-gtfs runtime helpers. The add-time build/seal pipeline
    // was removed with `add` (Memo 099 / Memo 152 PRD-016); only the runtime
    // read path survives (#maybeCallSqliteGtfsAutoTool reads the seal cache).
    // NOTE: no producer writes that cache anymore — the auto-tool runtime is
    // currently dormant (extracted to SqliteGtfsRuntime in PRD-019).
    // ---------------------------------------------------------------------
...
`
    ...
    static async doctor( { cwd } ) {
        ...
        const result = FlowMcpCli.#doctorResult( { cliName, cliVersion, checks, 'fix': libFix } )
